...
class Unifri1():  
  ...
  def UnifriGetOrders1(self):
    ...
    unifriorderj1 = requests.post("https://m.client.10010.com/welfare-mall-front/mobile/api/bj2402/v1",headers=unifriheaders1,params=unifridata1,cookies=unifricookies1,timeout=float(linecache.getline(r"unifri1cfg.set",26).strip())).json()
  ...
  def UnifriOrdering1(self):
    try:
      unifriftime1 = linecache.getline(r"unifri1cfg.set",23).strip()
      unifriftimes1 = 1
      ...
      if int(linecache.getline(r"unifri1cfg.set",29).strip()) == 1:
        times = time.strftime("%H{}%M{}%S{}").format("时","分","秒")   #加入时间,避免造成重复消息导致Server酱无法推送
        requests.get("https://sc.ftqq.com/%s.send?text=%s %s 下单成功,请尽快在30分钟内支付,逾期将失效哦"%(linecache.getline(r"unifri1cfg.set",31).strip(),times,unifrigoodsn1))
      AllinOneExit1()
    except:
      print("可能网络出错了,将重新运行")
  ...
  def UnifriMain1(self):
    ...
    unifricookies1 = {"Cookies":"%s"%(linecache.getline(r"unifri1cfg.set",34).strip())}
    ...
    for files in os.walk(os.getcwd()):
      if re.findall("Unifri1的商品 %s.*\.ordered"%(unifrigoodsn1),str(files),flags=re.I):
        ...
        AllinOneExit1()
    unifridata1 = 'reqsn=&reqtime=&cliver=&reqdata={"goodsId":"%s","payWay":"01","amount":"%s","saleTypes":"C","points":"0","beginTime":"","imei":"undefined","sourceChannel":"","proFlag":"","scene":"","pormoterCode":"","maxcash":"","floortype":"undefined"}'%(unifrigoodsid1,unifripaypri1)
    self.UnifriGetOrders1()
...
class Citic3651():
  def Citic365LocalGoods1(self):
    ...
    if citic365skuid1 == "37202008130178662128":
      citic365skun1 = "必胜客100"
    elif citic365skuid1 == "37202008171706774789":
      citic365skun1 = "呷哺呷哺50"
